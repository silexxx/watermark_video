...
#comment this and uncomment up to read from live cam
#cap=cv2.VideoCapture("video.mp4")

save_path = 'watermark.mp4'
frames_per_seconds = 24
config = CFEVideoConf(cap, filepath=save_path, res='420p')
fourcc = cv2.VideoWriter_fourcc(*'MP4V')
fourcc = cv2.VideoWriter_fourcc(*'XVID')
out = cv2.VideoWriter(save_path, config.video_type, frames_per_seconds, config.dims,fourcc)
